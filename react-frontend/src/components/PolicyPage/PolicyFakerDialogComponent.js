...
import policyFakerFactory from "./policyFakerFactory";
...
const PolicyFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = policyFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("policy").create(elem));
...
export default connect(null, mapDispatch)(PolicyFakerDialogComponent);
