...
import claimFakerFactory from "./claimFakerFactory";
...
const ClaimFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = claimFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("claim").create(elem));
...
export default connect(null, mapDispatch)(ClaimFakerDialogComponent);
