...
import trackingFakerFactory from "./trackingFakerFactory";
...
const TrackingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = trackingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tracking").create(elem));
...
export default connect(null, mapDispatch)(TrackingFakerDialogComponent);
